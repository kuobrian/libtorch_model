...
Detector::Detector(const torch::DeviceType& device_type) : device_(device_type){ }
...
void Detector::debug(std::string s) { std::cout<< s << std::endl; }
...
std::vector<std::vector<Detection>> Detector::Run(const cv::Mat& img,
    ...
    std::vector<float> pad_info = LetterboxImage(dst, dst, cv::Size(640, 640));
    const float pad_w = pad_info[0];
    const float pad_h = pad_info[1];
    const float scale = pad_info[2];

    ImagePreprocess(dst, dst);
    ...
    auto result = PostProcessing(detections, pad_w, pad_h, scale,
                                img.size(), conf_thr, iou_thr);
...
void Detector::ImagePreprocess(const cv::Mat& src, cv::Mat& dst) {
    dst = src.clone();
